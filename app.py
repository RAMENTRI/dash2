...
def get_video_metrics(youtube_analytics, start_date, end_date):
    ...
    st.write("🔍 Raw API Response:", result)  # Debug: show response
...
if selected_account:
    try:
        ...
        if video_df.empty:
            st.warning("⚠️ No analytics data found. Make sure this account has a YouTube channel with videos.")
        else:
